...
typedef struct FSAudioUserData {
    UInt32 mChannels; // 声道
    UInt32 mDataSize; // 数据大小
    void *mData;      // 数据
    AudioStreamPacketDescription mPacketDesc; // 音频流数据包描述
} FSAudioUserData;

@interface FSAudioDecoder () {
...
@property (nonatomic, strong) dispatch_queue_t decoderQueue; // 解码队列.
@property (nonatomic, assign) BOOL isError;

@end

@implementation FSAudioDecoder
...
- (instancetype)init {
    ...
    if (self) {
        // 创建解码队列
        _decoderQueue = dispatch_queue_create("com.louis.audioDecoder", DISPATCH_QUEUE_SERIAL);
    }
...
static OSStatus inputDataProcess(AudioConverterRef inConverter, UInt32 *ioNumberDataPackets, AudioBufferList *ioData, AudioStreamPacketDescription **outDataPacketDescription, void *inUserData) {
    FSAudioUserData *userData = (FSAudioUserData *) inUserData;
