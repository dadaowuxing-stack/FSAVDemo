...
vertex RasterizerData vertexShader(uint vertexID [[vertex_id]],
                                   constant FSVertex *vertexArray [[buffer(FSVertexInputIndexVertices)]]) {
    RasterizerData out;
...
// FSFragmentTextureIndexTextureY、FSFragmentTextureIndexTextureUV：表示纹理索引。
fragment float4 yuvSamplingShader(RasterizerData input [[stage_in]],
                                  ...
                                  texture2d<float> textureUV [[texture(FSFragmentTextureIndexTextureUV)]],
                                  constant FSConvertMatrix *convertMatrix [[buffer(FSFragmentInputIndexMatrix)]]) {
...
// [[texture(index)]]：纹理修饰符；可以加索引：[[texture(0)]] 对应纹理 0，[[texture(1)]] 对应纹理 1；
// FSFragmentTextureIndexTextureRGB：表示纹理索引。
fragment float4 rgbSamplingShader(RasterizerData input [[stage_in]],
                                  texture2d<half> colorTexture [[texture(FSFragmentTextureIndexTextureRGB)]]) {
    constexpr sampler textureSampler (mag_filter::linear, min_filter::linear);
