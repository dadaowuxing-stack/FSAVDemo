...
#define FSOutputBus 0

@interface FSAudioRender ()
...
- (instancetype)initWithChannels:(NSInteger)channels bitDepth:(NSInteger)bitDepth sampleRate:(NSInteger)sampleRate {
    ...
    if (self) {
        ...
        _renderQueue = dispatch_queue_create("com.louis.audioRender", DISPATCH_QUEUE_SERIAL);
    }
...
- (void)startPlaying {
    ...
    dispatch_async(_renderQueue, ^{
        ...
        if (status != noErr) {
            // 捕捉并回调开始渲染时的错误.
            [weakSelf _callBackError:[NSError errorWithDomain:NSStringFromClass([FSAudioRender class]) code:status userInfo:nil]];
        }
    });
}
...
- (void)_setupAudioRenderInstance:(NSError**)error {
    ...
    UInt32 flag = 1;
    status = AudioUnitSetProperty(_audioRenderInstance, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, FSOutputBus, &flag, sizeof(flag));
    if (status != noErr) {
        *error = [NSError errorWithDomain:NSStringFromClass(self.class) code:status userInfo:nil];
        return;
    }
    ...
    status = AudioUnitSetProperty(_audioRenderInstance, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, FSOutputBus, &inputFormat, sizeof(inputFormat));
    if (status != noErr) {
        *error = [NSError errorWithDomain:NSStringFromClass(self.class) code:status userInfo:nil];
        return;
    }
    ...
    status = AudioUnitSetProperty(_audioRenderInstance, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Global, FSOutputBus, &renderCallbackRef, sizeof(renderCallbackRef));
    if (status != noErr) {
        *error = [NSError errorWithDomain:NSStringFromClass(self.class) code:status userInfo:nil];
        return;
    }
...
static OSStatus audioRenderCallback(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inOutputBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    ...
    FSAudioRender *audioRender = (__bridge FSAudioRender *) inRefCon;
