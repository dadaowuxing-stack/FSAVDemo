...
//  FSAVDemo
//
//  Created by louis on 2022/7/18.
//

#import "FSVideoDecoder.h"
#import <VideoToolBox/VideoToolBox.h>

#define FSDecoderRetrySessionMaxCount 5
#define FSDecoderDecodeFrameFailedMaxCount 20

@interface FSVideoDecoderInputPacket : NSObject

@property (nonatomic, assign) CMSampleBufferRef sampleBuffer;

@end

@implementation FSVideoDecoderInputPacket

@end

@interface FSVideoDecoder ()
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _decoderQueue = dispatch_queue_create("com.louis.videoDecoder", DISPATCH_QUEUE_SERIAL);
...
- (void)decodeSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    if (!sampleBuffer || self.retrySessionCount >= FSDecoderRetrySessionMaxCount || self.decodeFrameFailedCount >= FSDecoderDecodeFrameFailedMaxCount) {
        return;
    }
    ...
    dispatch_async(_decoderQueue, ^{
        ...
        if (!weakSelf.decoderSession) {
            // 重试超过 FSDecoderRetrySessionMaxCount 次仍然失败则认为创建失败，报错.
            CFRelease(sampleBuffer);
            dispatch_semaphore_signal(weakSelf.semaphore);
            if (weakSelf.retrySessionCount >= FSDecoderRetrySessionMaxCount && weakSelf.errorCallBack) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    weakSelf.errorCallBack([NSError errorWithDomain:NSStringFromClass([FSVideoDecoder class]) code:setupStatus userInfo:nil]);
                });
            }
            return;
        }
        ...
        if (decodeStatus == kVTInvalidSessionErr) {
            ...
            if (setupStatus == noErr) {
                ...
                for (FSVideoDecoderInputPacket *packet in weakSelf.gopList) {
            ...
            // 解码当前帧失败.
            NSLog(@"FSVideoDecoder decode error:%d", decodeStatus);
        }
        ...
        FSVideoDecoderInputPacket *packet = [FSVideoDecoderInputPacket new];
        ...
        // 解码失败次数超过 FSDecoderDecodeFrameFailedMaxCount 次，报错.
        if (weakSelf.decodeFrameFailedCount >= FSDecoderDecodeFrameFailedMaxCount && weakSelf.errorCallBack) {
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.errorCallBack([NSError errorWithDomain:NSStringFromClass([FSVideoDecoder class]) code:decodeStatus userInfo:nil]);
            });
        }
    });
}
...
- (void)_clearCompressQueue {
    // 清空当前 GOP 缓冲区.
    for (FSVideoDecoderInputPacket *packet in self.gopList) {
...
static void decompressionOutputCallback( void *decompressionOutputRefCon, void *sourceFrameRefCon, OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef imageBuffer, CMTime presentationTimeStamp, CMTime presentationDuration ) {
    ...
    if (infoFlags & kVTDecodeInfo_FrameDropped) {
        NSLog(@"FSVideoDecoder drop frame");
        return;
    }
    
    // 向外层回调解码数据.
    FSVideoDecoder *videoDecoder = (__bridge FSVideoDecoder *) decompressionOutputRefCon;
