...
#import "FSYUVFrameCopier.h"
#import "FSContrastEnhancerFilter.h"
#import "FSDirectPassRenderer.h"
...
@implementation FSVideoOutput
{
    ...
    FSYUVFrameCopier*                         _videoFrameCopier;
    
    FSBaseEffectFilter*                       _filter;
    
    FSDirectPassRenderer*                     _directPassRenderer;
}
...
- (id)initWithFrame:(CGRect)frame textureWidth:(NSInteger)textureWidth textureHeight:(NSInteger)textureHeight  shareGroup:(EAGLSharegroup *)shareGroup {
    ...
    if (self) {
        ...
        _renderOperationQueue.name = @"com.louis.FSAVDemo.videoRenderQueue";
        
        __weak FSVideoOutput *weakSelf = self;
        [_renderOperationQueue addOperationWithBlock:^{
            if (!weakSelf) {
                return;
            }

            __strong FSVideoOutput *strongSelf = weakSelf;
            if (shareGroup) {
            ...
            strongSelf->_directPassRenderer = [[FSDirectPassRenderer alloc] init];
...
{
    return [[FSContrastEnhancerFilter alloc] init];
}
...
- (void)createCopierInstance {
    _videoFrameCopier = [[FSYUVFrameCopier alloc] init];
}
