...
@interface FSAVSynchronizer () {
    ...
    FSVideoFrame*                                       _currentVideoFrame;
...
static void* runDecoderThread(void* ptr) {
    FSAVSynchronizer* synchronizer = (__bridge FSAVSynchronizer*)ptr;
    [synchronizer run];
    return NULL;
}
...
- (instancetype)initWithPlayerStateDelegate:(id<FSPlayerStateDelegate>)playerStateDelegate {
    self = [super init];
    if (self) {
...
- (FSOpenState)openFile:(NSString *)path
    ...
    return [self openFile:path parameters:parameters error:error];
}
...
- (FSOpenState)openFile:(NSString *)path
             parameters:(NSDictionary*)parameters
                  error: (NSError **)error {
    //1、创建decoder实例
    _decoder = [[FSAVDecoder alloc] init];
    //2、初始化成员变量
    ...
    BOOL openCode = [_decoder openFile:path parameter:parameters error:error];
...
- (void)startDecoderThread {
    NSLog(@"FSAVSynchronizer::startDecoderThread ...");
    isOnDecoding = true;
    isDestroyed = false;
        ...
        while (numFrames > 0) {
            if (!_currentAudioFrame) {
                ...
                @synchronized(_audioFrames) {
                    ...
                    if (count > 0) {
                        FSAudioFrame *frame = _audioFrames[0];
...
- (void)checkPlayState {
    ...
    if (1 == _decodeVideoErrorState) {
        ...
        if (_decodeVideoErrorTotalTime > TIMEOUT_DECODE_ERROR) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if(self.playerStateDelegate && [self.playerStateDelegate respondsToSelector:@selector(restart)]){
                    [self.playerStateDelegate restart];
                }
            });
        }
        return;
    ...
    if (_buffered) {
        ...
        if (_bufferedTotalTime > TIMEOUT_BUFFER) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
#ifdef DEBUG
                NSLog(@"FSAVSynchronizer restart after timeout");
#endif
                if(self.playerStateDelegate && [self.playerStateDelegate respondsToSelector:@selector(restart)]){
                    NSLog(@"=============================== FSAVSynchronizer restart");
                    [self.playerStateDelegate restart];
                }
            });
            return;
...
- (BOOL)addFrames:(NSArray *)frames duration:(CGFloat)duration {
    if (_decoder.validVideo) {
        @synchronized(_videoFrames) {
            for (FSFrame *frame in frames)
                if (frame.type == FSFrameTypeVideo) {
    ...
    if (_decoder.validAudio) {
        @synchronized(_audioFrames) {
            for (FSFrame *frame in frames)
                if (frame.type == FSFrameTypeAudio) {
...
- (void)closeDecoder {
    if(_decoder){
        [_decoder closeFile];
        if(self.playerStateDelegate && [self.playerStateDelegate respondsToSelector:@selector(statisticsCallback:)]){
            [self.playerStateDelegate statisticsCallback:[_decoder getStatistics]];
        }
        _decoder = nil;
    }
}
...
- (void)destroyDecoderThread {
    NSLog(@"FSAVSynchronizer::destroyDecoderThread ...");
...
- (void)dealloc{
    NSLog(@"FSAVSynchronizer Dealloc...");
}
