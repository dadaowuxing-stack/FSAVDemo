...
@implementation FSFrame

@end

@implementation FSAudioFrame

@end

@implementation FSVideoFrame
...
static void avStreamFPSTimeBase(AVStream *st, CGFloat defaultTimeBase, CGFloat *pFPS, CGFloat *pTimeBase) {
    ...
    AVRational time_base = params->sample_aspect_ratio;
    
    if (st->time_base.den && st->time_base.num)
        timebase = av_q2d(st->time_base);
    else if(time_base.den && time_base.num)
        timebase = av_q2d(time_base);
    else
...
@interface FSAVDecoder (){
    AVFrame            *_videoFrame; // 视频帧
    AVFrame            *_audioFrame; // 音频帧
...
@implementation FSAVDecoder

static int interrupt_callback(void *ctx) {
    if (!ctx)
        return 0;
    __unsafe_unretained FSAVDecoder *p = (__bridge FSAVDecoder *)ctx;
...
- (BOOL)openFile:(NSString *)path
    ...
    _isSubscribe = YES;
    _statistics = [[FSStatistics alloc] init];
    _statistics.bufferStatusRecords = [[NSMutableArray alloc] init];
    _readLastestFrameTime = [[NSDate date] timeIntervalSince1970];
    avformat_network_init();
    //av_register_all();
    _statistics.beginOpen = [[NSDate date] timeIntervalSince1970] * 1000;
    int openInputErrCode = [self openInput:path parameter:parameters];
    if(openInputErrCode > 0) {
        _statistics.successOpen = ([[NSDate date] timeIntervalSince1970] * 1000 - _statistics.beginOpen) / 1000.0f;
        _statistics.failOpen = 0.0f;
        _statistics.failOpenType = 1;
        ...
        _statistics.failOpen = ([[NSDate date] timeIntervalSince1970] * 1000 - _statistics.beginOpen) / 1000.0f;
        _statistics.successOpen = 0.0f;
        _statistics.failOpenType = openInputErrCode;
        ret = NO;
    }
    _statistics.retryTimes = _connectionRetry;
    if(ret){
        ...
        while(((videoWidth <= 0 || videoHeight <= 0) && retryTimes > 0)){
            ...
            usleep(500 * 1000);
            _connectionRetry = 0;
            ret = [self openFile:path parameter:parameters error:error];
            if(!ret){
                //如果打开失败 则退出
                break;
            }
...
- (NSArray *)decodeFrames:(CGFloat)minDuration decodeVideoErrorState:(int *)decodeVideoErrorState {
    ...
    while (!finished) {
        ...
        if (pktStreamIndex ==_videoStreamIndex) {
            ...
            FSVideoFrame* frame = [self decodeVideo:packet packetSize:pktSize decodeVideoErrorState:decodeVideoErrorState];
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    FSAudioFrame * frame = [self handleAudioFrame];
                    if (frame) {
                        [result addObject:frame];
...
- (FSVideoFrame *)handleVideoFrame {
    if (!_videoFrame->data[0])
        return nil;
    FSVideoFrame *frame = [[FSVideoFrame alloc] init];
...
- (FSAudioFrame *)handleAudioFrame {
    ...
    FSAudioFrame *frame = [[FSAudioFrame alloc] init];
    frame.position = av_frame_get_best_effort_timestamp(_audioFrame) * _audioTimeBase;
    frame.duration = av_frame_get_pkt_duration(_audioFrame) * _audioTimeBase;
    frame.samples = pcmData;
    frame.type = FSFrameTypeAudio;
...
{
    if (_statistics.failOpenType == 1) {
        _statistics.firstScreenTimeMills = ([[NSDate date] timeIntervalSince1970] * 1000 - _statistics.beginOpen) / 1000.0f;
    }
}

- (void) addBufferStatusRecord:(NSString*) statusFlag
{
    if ([@"F" isEqualToString:statusFlag] && [[_statistics.bufferStatusRecords lastObject] hasPrefix:@"F_"]) {
        return;
    }
    float timeInterval = ([[NSDate date] timeIntervalSince1970] * 1000 - _statistics.beginOpen) / 1000.0f;
    [_statistics.bufferStatusRecords addObject:[NSString stringWithFormat:@"%@_%.3f", statusFlag, timeInterval]];
}
...
- (void) closeFile;
{
    ...
    if (_statistics.failOpenType == 1) {
        _statistics.duration = ([[NSDate date] timeIntervalSince1970] * 1000 - _statistics.beginOpen) / 1000.0f;
    }
    [self interrupt];
