...
@interface FSVideoCapture ()<AVCaptureVideoDataOutputSampleBufferDelegate>

@property (nonatomic, strong, readwrite) FSVideoCaptureConfig *config;
...
- (instancetype)initWithConfig:(FSVideoCaptureConfig *)config {
    ...
    if (self) {
        _config = config;
        _captureQueue = dispatch_queue_create("com.louis.videoCapture", DISPATCH_QUEUE_SERIAL);
...
- (void)_updateMirror {
    ...
    if ([connection isVideoMirroringSupported]) {
        if ((self.config.mirrorType & FSVideoCaptureMirrorFront) && self.config.position == AVCaptureDevicePositionFront) {
            connection.videoMirrored = YES;
        } else if ((self.config.mirrorType & FSVideoCaptureMirrorBack) && self.config.position == AVCaptureDevicePositionBack) {
            connection.videoMirrored = YES;
        } else {
